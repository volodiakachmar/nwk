...
<section class="container mx-auto px-6 my-1 flex flex-wrap -m-4">
    <div class="p-2 md:w-40 ">
        <a href="{{route('posts.create')}}" class="flex items-center p-4 bg-blue-200 rounded-lg shadow-xs cursor-pointer hover:bg-blue-500 hover:text-gray-100">
    ...
    <div class="p-2 md:w-40 ">
        <a href="{{route('praca.create')}}" class="flex items-center p-4 bg-gray-200 rounded-lg shadow-xs cursor-pointer hover:bg-gray-500 hover:text-gray-100">
    ...
    <div class="p-2 md:w-40 ">
        <a href="{{route('events.create')}}" class="flex items-center p-4 bg-indigo-200 rounded-lg shadow-xs cursor-pointer hover:bg-indigo-500 hover:text-gray-100">
...
<nav class="flex items-center justify-between flex-wrap bg-teal p-6">
    ...
    <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto">
        <div class="text-sm lg:flex-grow">
            <a href="#responsive-header" class="block mt-4 lg:inline-block lg:mt-0 text-teal-lighter  mr-4">
                Home
            </a>
            <a href="#responsive-header" class="block mt-4 lg:inline-block lg:mt-0 text-teal-lighter  mr-4">
                Articles
            </a>
            <a href="{{url('praca')}}" class="block mt-4 lg:inline-block lg:mt-0 text-teal-lighter">
                Works
            </a>
            <a href="" class="block mt-4 lg:inline-block lg:mt-0 text-teal-lighter">
                Events
            </a>
        </div>
    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    @foreach($events as $event)
                        <tr>
                            <td class="px-6 py-4 whitespace-no-wrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-12 w-12">
                                        <img class="h-13 w-13 rounded-full" src="{{asset('storage/'. $event->image_3_rounded)}}" alt="">
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-xl leading-5 font-medium text-gray-900">
                                            <a href="{{route('events.show', $event)}}">{{substr($event->event_title, 0, 50)}} ... </a>
                                        </div>
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap">
                                <div class="text-sm leading-5 text-gray-900">{{$event->event_location}}</div>
                                <div class="text-sm leading-5 text-gray-500">{{ $event->event_date }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                 active
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                @if(Auth::check())
                                    <a href="{{route('events.show', $event)}}" class="text-indigo-600 hover:text-indigo-900 px-3">Show</a>
                                @endif
                                @if(Auth::check())
                                    <form method="POST" action="{{route('events.destroy', $event)}}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" href="" class="text-indigo-600 hover:text-indigo-900 px-3">Delete</button>
                                    </form>
                                @endif
                                @if(Auth::check())
                                    <a href="{{route('events.edit', $event)}}" class="text-indigo-600 hover:text-indigo-900 px-3">Edit</a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
